from PyQt5.QtWidgets import (
    ...
    QMessageBox, QCalendarWidget, QWidget, QTableWidget, QTableWidgetItem,
)
...
class MuscleOverlayWidget(QWidget):
    ...
    def highlight_muscles(self, muscle_list):
        # 肌群正背面分类
        front_muscles = {"chest", "abs", "biceps", "shoulders", "legs", "calves"}
        back_muscles = {"back", "triceps", "glutes", "legs", "calves"}
...
class ViewPage(BasePage):
    def __init__(self, controller):
        ...
        self.table.setHorizontalHeaderLabels(["动作", "组数"])
    ...
    def on_date_selected(self, qdate: QDate):
        ...
        self.muscle_overlay.highlight_muscles([])
        ...
        if not matched:
            ...
            self.table.setItem(0, 0, QTableWidgetItem("该日暂无训练记录"))
            self.table.setSpan(0, 0, 1, 2)  # 合并两列
            return
        ...
        for info in matched:
            for ex, n_group in zip(info.exercises, info.n_group):
                rows.append((ex, str(n_group)))
                trained_parts.update(muscle_map.get(ex, []))

        self.table.setRowCount(len(rows))
        for i, (ex, n_group) in enumerate(rows):
            self.table.setItem(i, 0, QTableWidgetItem(ex))
            self.table.setItem(i, 1, QTableWidgetItem(n_group))
